...
export default function UsersDashboard() {
  ...
  const fetchUsersData = async (refresh = false) => {
    if (refresh) {
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              Registration trend and latest user accounts.
            </p>
          </div>

          <button
            type="button"
            onClick={() => void fetchUsersData(true)}
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-base sm:text-lg font-semibold text-slate-900">Registration Trend</h2>
        ...
        <div className="h-[280px] sm:h-[320px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={registrationData} margin={{ top: 8, right: 8, left: -10, bottom: 0 }}>
              ...
              <Tooltip />
              <Bar dataKey="count" fill="#A18167" radius={[4, 4, 0, 0]} name="Registrations" />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </section>
      ...
      <section className="hidden sm:block rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        <div className="px-4 py-4 border-b border-slate-200 sm:px-6">
          <h2 className="text-base sm:text-lg font-semibold text-slate-900">Latest Registered Users</h2>
        </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {users.map((user) => (
                <tr key={user.id} className="hover:bg-slate-50/70">
                  <td className="px-4 py-3 font-medium text-slate-900">{user.name || "Unnamed User"}</td>
                  <td className="px-4 py-3 text-slate-600">{user.email || "No email"}</td>
      ...
      <section className="sm:hidden">
        ...
        <div className="space-y-2">
          {users.slice(0, 12).map((user) => (
            <article key={`mobile-${user.id}`} className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
              <p className="text-sm font-semibold text-slate-900">{user.name || "Unnamed User"}</p>
              <p className="text-xs text-slate-600 mt-1 break-all">{user.email || "No email"}</p>
