  ...
  try {
    ...
    if (!reason) {
      return NextResponse.json(
        { error: "Please describe what you want the guide to change." },
        { status: 400 }
      );
    }
    ...
    if (booking.status !== "PENDING") {
      return NextResponse.json(
        { error: "Only pending bookings can request changes from the guide" },
        { status: 400 }
      );
    }
    ...
    if (hasPendingRequest) {
      return NextResponse.json(
        { error: "A guide request is already pending for this booking" },
        { status: 400 }
      );
    }
    ...
    const notification = await (prisma as any).notification.create({
      data: {
        ...
        actorId: session.user.id,
        type: "booking_change_request",
        message: `${booking.user.name || booking.user.email || "A user"} requested a booking change.`,
        relatedPostId: booking.id,
